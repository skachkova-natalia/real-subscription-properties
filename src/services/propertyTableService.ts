...
export const propertyTableService = bindAllMethods({
  async getAvailableSubstance(): Promise<SubstanceFiltersResponse> {
    return (
      await axiosApiInstance.get(`https://rsp-api.online/getAvailableSubstances`)
    ).data;
  },
  async getCalcModesInfo(id: string): Promise<ModesResponse> {
    return (
      await axiosApiInstance.get(`https://rsp-api.online/getCalcModesInfo?id=${id}`)
    ).data;
  },
  async getTable(substanceId: string, modeId: string): Promise<void> {
    return (
      await axiosApiInstance.get(`https://rsp-api.online/getPropertiesTable?substanceId=${substanceId}&modeId=${modeId}`)
    ).data;
  },
});
